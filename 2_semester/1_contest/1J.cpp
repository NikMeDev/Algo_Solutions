...
int GetNeed(
    ...
    std::set<int>& avoid, std::vector<std::pair<int, int>>& edges) {
  ...
  std::vector<int> condensed(state.count, 0);
  for (int id : avoid) {
    VType start = edges[id].first;
    VType end = edges[id].second;
    condensed[state.result[start]]++;
    condensed[state.result[end]]++;
  }
  int full_count = 0;
  for (auto vertex : condensed) {
    if (vertex == 1) {
      full_count++;
    }
  }
  return (full_count / 2) + (full_count % 2);
}
...
int main() {
  ...
  for (int i = 1; i <= edge_count; i++) {
    ...
    dict[first][second].first = i - 1;
    dict[second][first].first = i - 1;
  ...
  std::cout << GetNeed(graph, dict, set, edges);
  return 0;
}